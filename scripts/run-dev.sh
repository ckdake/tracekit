...
# tracekit Web App Development Server (moved from repo root)
...
if [ ! -f "tracekit_config.json" ]; then
    ...
    echo "   Please run this script from the tracekit project root directory or mount your config to /app/tracekit_config.json"
    exit 1
fi
